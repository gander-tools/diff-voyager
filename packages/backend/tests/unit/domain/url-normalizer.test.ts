...
describe("UrlNormalizer", () => {
	describe("normalize", () => {
		it("should remove trailing slash", () => {
			expect(UrlNormalizer.normalize("https://example.com/page/")).toBe(
				"/page",
			);
		});
		...
		it("should sort query parameters", () => {
			expect(UrlNormalizer.normalize("https://example.com/page?b=2&a=1")).toBe(
				"/page?a=1&b=2",
			);
		});
		...
		it("should remove fragment", () => {
			expect(UrlNormalizer.normalize("https://example.com/page#section")).toBe(
				"/page",
			);
		});
		...
		it("should handle multiple tracking params", () => {
			expect(
				UrlNormalizer.normalize(
					"https://example.com/page?utm_source=a&utm_medium=b&real=value",
				),
		...
		it("should handle path without query", () => {
			expect(
				UrlNormalizer.normalize("https://example.com/some/deep/path"),
			).toBe("/some/deep/path");
		});
	});
	...
	describe("getOrigin", () => {
		it("should extract origin from URL", () => {
			expect(UrlNormalizer.getOrigin("https://example.com/page?q=1")).toBe(
				"https://example.com",
			);
		});

		it("should include port if non-standard", () => {
			expect(UrlNormalizer.getOrigin("http://localhost:3000/page")).toBe(
				"http://localhost:3000",
			);
		});
	});
