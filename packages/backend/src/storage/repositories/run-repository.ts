...
export class RunRepository {
  ...
  async findByProjectId(projectId: string): Promise<RunEntity[]> {
    const stmt = this.db.prepare('SELECT * FROM runs WHERE project_id = ? ORDER BY created_at DESC');
  ...
  async updateStatistics(id: string, statistics: RunStatistics): Promise<void> {
    const stmt = this.db.prepare('UPDATE runs SET statistics_json = ? WHERE id = ?');
