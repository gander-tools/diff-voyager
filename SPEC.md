...
| T45 | .      | tests: `scraper.ts` regression — `page.goto` fires `response` w/ URL ≠ passed `url` (redirect sim), marked as main-frame nav request → assert `page.source.html` non-empty & equals mocked response body; extend `createFakePage`/`createFakeResponse` fakes w/ `mainFrame()`/`isNavigationRequest()`/`frame()`                                                                                         | V33                      |
| T46 | .      | `src/scraper.ts` — replace `response.url() === url` match w/ `response.request().isNavigationRequest() && response.frame() === page.mainFrame()`                                                                                                                                                                                                                                                        | V33                      |
