...
- files: `src/config.ts` pure `process.env` reads (DB path & env consts) + explicit opt-in `loadEnvFiles()` export (⊥ auto-call on import — V18) | `src/db.ts` connection factory, migration, typed query helpers | `src/cli.ts` commander CLI, URL zod-validated | `src/worker.ts` poll loop (500ms idle), atomic claim, scrape, persist | `src/scraper.ts` Playwright logic, typed result → JSON
...
- V17: config load order → `dotenv.config({path:'.env'})` → `dotenv.config({path:'.env.local', override:true})` → per-var `process.env` fallback (last resort); missing `.env`/`.env.local` files ⊥ fail, fallback still works; `loadEnvFiles()` invoked explicitly by each entrypoint (CLI/worker), ⊥ automatically on `config.ts` import
- V18: `src/config.ts` module top-level ⊥ perform file I/O or `process.env` mutation as import side-effect (deepen pass, 2026-07-01: removed auto-call that fired on every import incl. tests — dotenv README itself recommends configuring "as early as possible in your application", i.e. at the entrypoint, not inside a shared module); env loading only via explicit `loadEnvFiles()` call, invoked first line of each entrypoint (`src/cli.ts`, `src/worker.ts`)
...
| T8  | .      | `src/cli.ts` — `add <url>`, `load <file>`                                                                                                                                                      | V1,V10,V13,V18 |
...
| T16 | .      | `src/worker.ts` — poll loop, atomic claim, invoke scraper, write meta.json, cleanup+exit                                                                                                       | V2,V9,V18      |
