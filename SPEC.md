...
| T40 | .      | tests: `UrlRunsRepo` — claimNextPending atomic race (replaces raw-SQL test @ tests/worker.test.ts:106 w/ repo-level version), markDone, markFailed, countPending, listFailed, bulkInsert, deleteByRun                                                                                                                                                                                                   | V2,V6,V9,V31             |
| T41 | .      | `src/repos/urlRunsRepo.ts` — `UrlRunsRepo` interface + `DrizzleUrlRunsRepo` impl; atomic claim via `.update(urlRuns).set(...).where(and(eq(id,...),eq(status,'pending'))).returning()`                                                                                                                                                                                                                  | V2,V9                    |
