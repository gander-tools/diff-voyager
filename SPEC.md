...
| T27 |        | tests: `config.ts` dotenv load — captured stdout during CLI invocation contains ⊥ "injected env" tip lines                                                                                                                                                                                                                                                                                              | V27               |
| T28 |        | `src/config.ts` — add `quiet: true` to both `dotenv.config()` calls in `loadEnvFiles`                                                                                                                                                                                                                                                                                                                   | V27               |
| T29 | x      | `.gitignore` — add `data/`, `logs/`, `snapshots/`; docs/repo-hygiene, ⊥ test skeleton                                                                                                                                                                                                                                                                                                                   | §C                |
| T30 |        | tests: `scraper.ts` regression — scrape a real/local test page, assert `screenshot.*` + `meta.json` written, ⊥ `ReferenceError` thrown from `page.evaluate`                                                                                                                                                                                                                                             | V26               |
| T31 |        | `src/scraper.ts` — fix `page.evaluate(extractDom)` `__name is not defined` (avoid serializing bundler-transformed named fn to browser context)                                                                                                                                                                                                                                                          | V26               |
| T32 |        | tests: `worker.ts` `finalizeRun` — `failed` count>0 → `done_with_errors` status + `errors.json` written w/ `{url,error}` entries before `url_runs` delete; `failed` count=0 → unchanged `done` path                                                                                                                                                                                                     | V9                |
| T33 |        | `src/worker.ts` — implement `done_with_errors` status branch + `errors.json` write in `finalizeRun`                                                                                                                                                                                                                                                                                                     | V9                |
| T34 |        | tests: `clean` cmd — guard when run `open`; deletes `data/` (+ WAL/SHM), `SNAPSHOT_DIR`, `LOG_DIR`; next `openDb` call recreates `data/`                                                                                                                                                                                                                                                                | V25               |
| T35 |        | `src/cli.ts` — `clean` command                                                                                                                                                                                                                                                                                                                                                                          | V25               |
