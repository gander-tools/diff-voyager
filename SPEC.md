...
| T42 | .      | `src/worker.ts` — rewire thru `RunsRepo`/`UrlRunsRepo` per §C repo-injection-point; fn signatures db→repo params; rewrite `tests/worker.test.ts` call sites (repo-backed temp SQLite); pure behavior, ⊥ external behavior change                                                                                                                                                                        | V28,V30,V31,V32          |
