...
| T43 | .      | `src/cli.ts` — rewire all `db.prepare(...)` sites thru `RunsRepo`/`UrlsRepo`/`UrlRunsRepo` per §C repo-injection-point; fn signatures db→repo params; rewrite `tests/cli.test.ts`,`tests/cli-process.test.ts`,`tests/e2e.test.ts` call sites; pure behavior, ⊥ external behavior change                                                                                                                 | V28,V30,V31,V32          |
